...
keystone endpoint-create --region RegionOne --service_id $SERVICE_ID_IMAGE --publicurl "http://volume-node.example.org:8776/v1/$(tenant_id)s" --adminurl "http://volume-node.example.org:8776/v1/$(tenant_id)s" --internalurl "http://volume-node:8776/v1/$(tenant_id)s"
